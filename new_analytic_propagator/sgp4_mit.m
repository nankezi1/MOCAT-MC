...
if 98/XKMPER<(aopp*(1-eo)-1)<156/XKMPER
    ss=aopp*(1-eo)-s+aE;
    term=((qo-ss)+s-ss)^4;
elseif aopp*(1-eo)<98/XKMPER
    ss=20/XKMPER+aE;
    term=((qo-ss)+s-ss)^4;
else
    term=1.88027916e-9;
...
C3=(term*xi^5*A30*nopp*aE*sin(io))/(k2*eo);
...
Du=-(k2/(4*p_L^2))*(7*theta^2-1)*sin(2*u);
...
v=vkmpersec*r_dotk*U+vkmpersec*(rf_dotk)*V;
