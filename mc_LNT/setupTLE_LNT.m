...
function cfgMC = setupTLE_LNT(rngseed)
    % SCENARIO: Initial population is LEO TLEs from Aug 2022
    % https://conference.sdo.esoc.esa.int/proceedings/sdc6/paper/199
    
    % Start with current TLE's (18k objects)
    % Launches: 2001-2009 repeated (~ 75 / yr via Fig 2.14 in ESA 2022)
    ...
    % SCENARIO PARAMETERS
    cfgMC.PMD = 0.8;                   % post mission disposal (active sats only); ESA: 55% RB, 40% PL, 90% Const
    cfgMC.PMDconstel = 0.9;             
    cfgMC.PMDrb = 0.55;
    
    cfgMC.alph = 0.01;                 % active control failure probability with only one object active
%     cfgMC.rad2pLNTfunc = @(x) 1./(1+exp(-25*(x - 0.3))); % curve for alph effectiveness for LNT objects; input: radius
    ...
    cfgMC.alph_a = 0;                   % active control failure  with both active
    ...
    cfgMC.launchRepeatYrs = [2022.5, 2022.99];    % Min/max year of obj to repeatedly launch (inclusive)
    ...
    % cfgMC.launchMinPerShell = 10;           % [integer >= 0] min number of launches (payload) per shell as specified in paramSSEM.R02
        ...
        % Constellation
        cfgMC.constellationFile = 'megaconstellationLaunches.xlsx'; 
        cfgMC.constellationSheet = 'forexport';  % all 54 constellations; use 'forexport2' for fewer constellations
...
%     cfgMC.CUBE_RES = mean(cfgMC.a_all(cfgMC.ap_all<(cfgMC.altitude_limit_up+radiusearthkm),1))*0.01; 
    ...
    % Save output file
    cfgMC.save_diaryName = 'test';          % save commandline output text to this output;  to skip saving: ''
...
%   time0 = datetime(jdsatepoch_sat1,'convertfrom','juliandate'); % [datetime]
    ...
    ind_matsat_old = jd2date(mat_sats(:, idx_launch_date)) < 2015;  % then turn off old payloads    
    ...
    switch launch_model
        case {'random','matsat'}
            ...
            if strcmp(launch_model,'matsat')
                ...
                repeatT1 = juliandate(datetime(floor(cfg.launchRepeatYrs(1)),1,1) ...
                    + days( (cfg.launchRepeatYrs(1) - floor(cfg.launchRepeatYrs(1))) * 365));
                repeatT2 = juliandate(datetime(floor(cfg.launchRepeatYrs(2)),1,1) ...
                    + days( (cfg.launchRepeatYrs(2) - floor(cfg.launchRepeatYrs(2))) * 365));
                launchwindow = (jds >= repeatT1) & (jds <= repeatT2);
                ...
                if ~isempty(cfg.constellationFile)
                    ...
                    fprintf('TLE launch repeat selected \n\t %i objects total over %0.1f years ... \n\t omitting %i constellations from repeatlaunches ...\n\t constellations are launched via cfg.constellation from %s\n',...
                    size(cfg.repeatLaunches,1), diff(cfg.launchRepeatYrs), sum(constellationIdx),cfg.constellationFile);
                else
                    fprintf('TLE launch repeat selected (%i objects total over %0.1f years)\n',...
                    sum(launchwindow), diff(cfg.launchRepeatYrs));
                end
            end
        case {'data','Somma'}
    ...
    % turn on constellation flag for Starlink and Oneweb\
                % mat_sats, repeatlaunch, additional_launches
        if ~isempty(cfg.constellationFile)
            % 260 kg objects as Starlink -- add to first Starlink constel
            staridx = cfg.constellation.constIndex(find(strcmp(cfg.constellation.Constellation,'Starlink'),1));
            if isempty(staridx)
               staridx = 1; % put in '1' as constellation index (misc) 
            end                        
            % 148 kg objects as Oneweb -- add to first Oneweb constel            
            onewebidx = cfg.constellation.constIndex(find(strcmp(cfg.constellation.Constellation,'OneWeb'),1));
            if isempty(onewebidx)
               onewebidx = 1; % put in '1' as constellation index (misc) 
            end            
        else % no megaconstellation xlsx file given
            staridx = 1;
            onewebidx = 1;
        end

        mat_sats(mat_sats(:,idx_mass) == 260 , idx_constel) = staridx;
        cfg.repeatLaunches(cfg.repeatLaunches(:,idx_mass) == 260, idx_constel) = staridx;
        additional_launches(additional_launches(:,idx_mass) == 260, idx_constel) = staridx;
        mat_sats(mat_sats(:,idx_mass) == 148 , idx_constel) = onewebidx;
        cfg.repeatLaunches(cfg.repeatLaunches(:,idx_mass) == 148, idx_constel) = onewebidx;
        additional_launches(additional_launches(:,idx_mass) == 148, idx_constel) = onewebidx;

    % add in mission lifetime (!!)
        mat_sats(mat_sats(:,idx_controlled)>0, idx_missionlife) = cfg.missionlifetime;
        cfg.repeatLaunches(cfg.repeatLaunches(:,idx_controlled)>0, idx_missionlife) = cfg.missionlifetime;
        additional_launches(additional_launches(:,idx_controlled)>0, idx_missionlife) = cfg.missionlifetime;

    % add in desired_a
        mat_sats(mat_sats(:,idx_controlled)>0, idx_a_desired) = mat_sats(mat_sats(:,idx_controlled)==1,idx_a);
        cfg.repeatLaunches(cfg.repeatLaunches(:,idx_controlled)>0, idx_missionlife) = cfg.repeatLaunches(cfg.repeatLaunches(:,idx_controlled)==1,idx_a);
        additional_launches(additional_launches(:,idx_controlled)>0, idx_missionlife) = additional_launches(additional_launches(:,idx_controlled)==1,idx_a);
...
function cfg = setupConstellation(cfg)
    if ~isempty(cfg.constellationFile)
        ...
        % add constellation index to be used in main_mc for idx_constellation
        tab.constIndex = [1:size(tab,1)]' + 1;    % idx_constellation = 1 reserved for unknown constellations (just in case)
        % add company info in tab.constCompany
        compID = zeros(size(tab,1),1);
        ucomp = unique(tab.Constellation);
        SLcomp = contains(unique(tab.Constellation), 'Starlink');
        curID = 2;
        for cc = 1:numel(ucomp)
            if SLcomp(cc) % if starlink 
                compID(contains(tab.Constellation, ucomp(SLcomp))) = 1;
            else
                compID(contains(tab.Constellation, ucomp(cc))) = curID;
                curID = curID + 1;
            end
%             curcomp = ucomp(cc);
        end
        tab.constCompanyID = compID;
    else
        tab = [];
    end
